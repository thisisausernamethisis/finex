...
import { hasAssetAccess } from '../rbac';
import { PrismaClient } from '@prisma/client';

/**
 * Middleware function to ensure that the current user is the owner of the specified asset.
 * 
 * @param req The Next.js request object
 * @param assetId The ID of the asset to check ownership for
...
export async function ensureOwner(
  ...
  // If the asset doesn't exist, return 404 Not Found
  if (!asset) {
    return NextResponse.json({ message: 'Asset not found' }, { status: 404 });
  }
  
  // Check if the user has access to the asset using the RBAC system
  // Need to map userId to ownerId to match the RBAC function signature
  const ok = await hasAssetAccess(user.id, {
